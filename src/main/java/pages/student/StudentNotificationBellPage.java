...
public class StudentNotificationBellPage extends BasePage {
    ...
    @FindBy(id = "student-layout-notifications-panel")
    private WebElement notificationsPanel;

    @FindBy(xpath = "//button[starts-with(@id,'student-layout-notification-')]")
    private List<WebElement> notificationItems;

    @FindBy(xpath = "//button[@id='student-layout-bell-btn']//span[contains(@class,'h-2') and contains(@class,'w-2') and contains(@class,'rounded-full')]")
    ...
    public void clickNotificationByName(String notificationName) {
        driver.findElement(By.id("student-layout-notification-" + notificationName)).click();
    }
